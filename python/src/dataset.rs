...
#[pyfunction]
pub fn parse_grid_dataset<'py>(py: Python<'py>, data: &[u8], drop_variables: Option<&PyList>) -> &'py PyDict {
    ...
    let var_names = vars.keys().cloned().collect::<Vec<String>>();
    ...
    for (_var, v) in var_mapping.iter() {
    ...
    for (_var, v) in var_mapping.iter() {
        ...
        for k in v.iter() {
            ...
            if let Some(vertical_value) =
                mapping.get(k).unwrap().2.first_fixed_surface_value
            {
    ...
    attrs.set_item("meta", "Generated with gribberishpy").unwrap();
